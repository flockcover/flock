...
module.exports = function(app) {
  ...
  app.get('/api/files/:path', function(req, res, next) {
    ...
    try {
      ...
      response.status = 500
      response.error = `Could not get files for ${req.params.path} - code [${
        err.code
      }]`
      response.internalError = err
      res.status(response.status).send(response)
  ...
  app.get('/api/files/:path/**', function(req, res, next) {
    ...
    try {
      ...
      response.status = 500
      response.error = `Could not get files for ${filesPath} - code [${
        err.code
      }]`
      response.internalError = err
      res.status(response.status).send(response)
  ...
  app.get('/api/file/:id', function(req, res, next) {
    ...
    const allDone = contents => {
      if (contents.error) {
        response.status = 500
        response.error = `Could not read file ${req.params.id} - code [${
          contents.error.code
        }]`
  ...
  app.get('/api/file/:id/**', function(req, res, next) {
    ...
    const allDone = contents => {
      if (contents.error) {
        response.status = 500
        response.error = `Could not read file ${filePath} - code [${
          contents.error.code
        }]`
        response.internalError = contents.error
        res.status(response.status).send(response)
  ...
  app.post('/api/file/:id/**', upload.array(), function(req, res, next) {
    ...
    const allDone = contents => {
      if (contents.error) {
        response.status = 500
        response.error = `Could not create file ${filePath} - code [${
          contents.error.code
        }]`
        response.internalError = contents.error
        res.status(response.status).send(response)
  ...
  app.put('/api/file/:id/**', upload.array(), function(req, res, next) {
    ...
    const allDone = contents => {
      if (contents.error) {
        response.status = 500
        response.error = `Could not update file ${filePath} - code [${
          contents.error.code
        }]`
        response.internalError = contents.error
        res.status(response.status).send(response)
  ...
  app.delete('/api/file/:id/**', function(req, res, next) {
    ...
    const allDone = contents => {
      if (contents.error) {
        response.status = 500
        response.error = `Could not delete file ${filePath} - code [${
          contents.error.code
        }]`
        response.internalError = contents.error
        res.status(response.status).send(response)
